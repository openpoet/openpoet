		...
		},
		{
			...
			InputSchema: ToolDefinitionInput{
				...
				Properties: map[string]ToolPropertySchema{
					...
					"isolation":                    {Type: "string", Description: "Let OpenPoet pick the working tree: 'auto' uses the project's main checkout while it is free and provisions an isolated git-worktree lane once another live session holds it; 'always' provisions a lane unconditionally (use when this work will touch the same files as another session); 'never' (default) always uses the main checkout. A lane is created on demand — no pre-provisioning needed — and the response carries its workspace_id for a later merge."},
				},
				Required: []string{"project_id"},
			},
			Context: ToolContextBoth,
